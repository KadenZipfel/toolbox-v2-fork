...
var SCROLL_STEPS_TO_FRAME_STEPS = new Map([
...
var RenderLoop = (function () {
    ...
    RenderLoop.prototype.setupScrollListener_ = function () {
        var _this = this;
        window.addEventListener('scroll', function () { return _this.runScrollLoopAndSetupListener_(); }, { passive: true, capture: false, once: true });
    };
    ...
    RenderLoop.prototype.runScrollLoop = function () {
        ...
        SCROLL_STEP_ORDER.forEach(function (step) {
            var frameStep = SCROLL_STEPS_TO_FRAME_STEPS.get(step);
            var fns = _this.scheduledFns_.get(step).values();
            var nextFn;
            while (nextFn = fns.next().value) {
                _this.addFnToStep_(nextFn, frameStep);
            }
            _this.scheduledFns_.set(step, new Map());
        });
    };
