...
var PhysicalSlide = (function () {
    ...
    PhysicalSlide.prototype.adjustSplit_ = function (carousel, target, adjustment) {
        ...
        var _a = PhysicalSlide.getHalves_(carousel, targetSlide), slidesBefore = _a[0], slidesAfter = _a[1];
        var activeIndex = carousel.getSlideIndex(activeSlide);
        var targetIndex = carousel.getSlideIndex(targetSlide);
        var increment = get_sign_1.getSign(activeIndex - targetIndex);
        var shiftFunction = increment == 1 ?
            function () { return slidesAfter.push(slidesBefore.shift()); } :
            function () { return slidesBefore.unshift(slidesAfter.pop()); };
        for (var i = targetIndex; i != activeIndex; i += increment) {
            shiftFunction();
        }
