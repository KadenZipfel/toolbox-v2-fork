...
var PhysicalSlide = (function () {
    ...
    PhysicalSlide.prototype.adjustSplit_ = function (carousel, target, rawAdjustment) {
        if (target === void 0) { target = null; }
        if (rawAdjustment === void 0) { rawAdjustment = zero_vector_2d_1.ZERO_VECTOR_2D; }
        ...
        var distanceFromCenter = get_visible_distance_between_element_centers_1.getVisibleDistanceBetweenElementCenters(targetSlide);
        var distanceFromCenterSign = get_sign_1.getSign(distanceFromCenter);
        var requiresReset = Math.abs(distanceFromCenter) > (totalWidth / 2);
        var adjustment = carousel.allowsLooping() && requiresReset ?
            new vector_2d_1.Vector2d(rawAdjustment.x - (totalWidth * distanceFromCenterSign), rawAdjustment.y) :
            rawAdjustment;
