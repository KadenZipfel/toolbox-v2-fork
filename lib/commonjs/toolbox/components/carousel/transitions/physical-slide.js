...
var PhysicalSlide = (function () {
    ...
    PhysicalSlide.prototype.adjustSplit_ = function (carousel, target, adjustment) {
        ...
        if (carousel.allowsLooping()) {
            ...
            var _b = PhysicalSlide.getHalves_(carousel, targetSlide), slidesBefore_1 = _b[0], slidesAfter_1 = _b[1];
            if (diff !== 0) {
                var shiftFunction = diff > 0 ?
                    function () { return slidesAfter_1.push(slidesBefore_1.shift()); } :
                    function () { return slidesBefore_1.unshift(slidesAfter_1.pop()); };
                var absDiff = Math.min(Math.abs(diff), slidesBefore_1.length, slidesAfter_1.length);
                for (var i = 0; i < absDiff; i++) {
                    shiftFunction();
                }
            }
            this.adjustSlidesBefore_(targetSlide, slidesBefore_1, adjustment);
            this.adjustSlidesAfter_(targetSlide, slidesAfter_1, adjustment);
        }
    };
