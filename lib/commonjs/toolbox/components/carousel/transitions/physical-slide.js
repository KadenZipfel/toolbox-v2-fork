...
var PhysicalSlide = (function () {
    ...
    PhysicalSlide.prototype.adjustSplit_ = function (carousel, target, adjustment) {
        ...
        var loopedTranslation = dynamic_default_1.DynamicDefaultMap.usingFunction(function () { return 0; });
        ...
        var slidesBefore = [];
        var slidesAfter = [];
        var slidesByDistance = carousel.getSlides()
            .reduce(function (slidesByDistance, slide) {
            if (slide !== targetSlide) {
                var distance = get_visible_distance_between_element_centers_1.getVisibleDistanceBetweenElementCenters(targetSlide, slide);
        ...
        var targetOffset = targetSlide.offsetWidth / 2;
        slidesBefore.forEach(function (slide) {
        ...
        slidesAfter.forEach(function (slide) {
            var halfWidth = -slide.offsetWidth / 2;
            var distance = get_visible_distance_between_element_centers_1.getVisibleDistanceBetweenElementCenters(targetSlide, slide);
            targetOffset += halfWidth;
            var difference = targetOffset - distance;
