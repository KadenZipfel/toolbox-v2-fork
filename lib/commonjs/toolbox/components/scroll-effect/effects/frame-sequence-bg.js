...
var union_1 = require("../../../utils/set/union");
...
var FrameSequenceBg = (function () {
    function FrameSequenceBg(frames, container, _a) {
        ...
        this.backupFrames_ = new array_1.ArrayMap();
    ...
    FrameSequenceBg.prototype.loadImage_ = function (imageUrlIndexToLoad) {
        ...
        return this.loadImageFunction_(frameUrl)
            ...
            set_styles_from_map_1.setStylesFromMap(_this.frameElements_[imageUrlIndexToLoad], new Map([_this.getBackgroundImageStyle_(imageUrlIndexToLoad)]));
            if (_this.lastState_) {
    ...
    FrameSequenceBg.prototype.run = function (target, distance, distanceAsPercent) {
        ...
        var targetFrame = Math.round(new numeric_range_1.NumericRange(0, frameCountWithInterpolation)
        ...
        if (this.lastTargetFrame_ === targetFrame) {
            if (this.zIndex_ >= this.zIndexCap_ && CURRENT_BROWSER === firefox_1.Firefox) {
                this.resetZIndexes_();
            }
        }
        else {
    ...
    FrameSequenceBg.prototype.resetZIndexes_ = function () {
        ...
        this.frameElements_.forEach(function (frameElement) {
            var newIndex = parseInt('0' + frameElement.style.zIndex) - _this.zIndexCap_;
    ...
    FrameSequenceBg.prototype.clearFrames_ = function (rawExceptions) {
        var _this = this;
        if (rawExceptions === void 0) { rawExceptions = null; }
        var framesToClear;
        var exceptions = CURRENT_BROWSER === firefox_1.Firefox && this.lastState_ !== null ?
            union_1.union(rawExceptions, new Set([this.lastState_.backFrame, this.lastState_.frontFrame])) :
            rawExceptions;
        if (exceptions) {
    ...
    FrameSequenceBg.prototype.replaceFramesInPlace = function (frames) {
        ...
        if (frames.length !== this.imageUrlsInOrder_.length) {
            throw new Error('Can only replaceFramesInPlace if frame count is constant');
        }
