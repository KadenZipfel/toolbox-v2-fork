...
var PhysicalSlide = (function () {
    ...
    PhysicalSlide.prototype.initActiveSlide_ = function (target, carousel) {
        var _this = this;
        renderLoop.measure(function () { return _this.transition(target, carousel); });
    };
    ...
    PhysicalSlide.prototype.applyDragAdjustment_ = function (carousel, target, dragAdjustment) {
        if (target === void 0) { target = null; }
        if (dragAdjustment === void 0) { dragAdjustment = ZERO_VECTOR_2D; }
        carousel.getSlides()
            .filter(function (slide) { return slide !== target; })
            .forEach(function (slide) { return translate2d(slide, dragAdjustment); });
    };
    PhysicalSlide.prototype.adjustSplit_ = function (carousel, target, dragAdjustment) {
        ...
        var slidesByDistance = reverseMap(distancesFromTarget);
        var _a = this.splitSlides_(slidesByDistance), slidesBefore = _a[0], slidesAfter = _a[1];
        this.adjustSlidesForSplit_(carousel, targetSlide, slidesBefore, distancesFromTarget, -1);
        this.adjustSlidesForSplit_(carousel, targetSlide, slidesAfter, distancesFromTarget, 1);
    ...
    PhysicalSlide.prototype.adjustSlidesForSplit_ = function (carousel, targetSlide, slides, distancesFromTarget, direction) {
        var _this = this;
        slides.forEach(function (slide) {
            _this.adjustSlideForSplit_(carousel, targetSlide, slide, distancesFromTarget, direction);
        });
    };
