...
var Carousel = (function () {
    ...
    Carousel.prototype.updateClasses_ = function (activeSlide) {
        ...
        var beforeClasses = this.slides_.map(function (slide, index) { return _this.beforeCssClass_ + "--" + index; });
        var afterClasses = this.slides_.map(function (slide, index) { return _this.afterCssClass_ + "--" + index; });
        var removeProblemClasses = function (slide) {
            beforeClasses.forEach(function (problemClass) { return removeClassIfPresent(slide, problemClass); });
            afterClasses.forEach(function (problemClass) { return removeClassIfPresent(slide, problemClass); });
        };
        slidesBefore
            .forEach(function (slide, index) {
            removeClassIfPresent(slide, _this.activeClass_);
            addClassIfMissing(slide, _this.beforeCssClass_);
            removeClassIfPresent(slide, _this.afterCssClass_);
            removeProblemClasses(slide);
            addClassIfMissing(slide, _this.beforeCssClass_ + "--" + index);
        });
        slidesAfter
            .forEach(function (slide, index) {
            removeClassIfPresent(slide, _this.activeClass_);
            removeClassIfPresent(slide, _this.beforeCssClass_);
            addClassIfMissing(slide, _this.afterCssClass_);
            removeProblemClasses(slide);
            addClassIfMissing(slide, _this.afterCssClass_ + "--" + index);
        });
    };
